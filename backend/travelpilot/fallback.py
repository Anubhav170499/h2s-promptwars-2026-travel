...
from travelpilot.models import (
    TravelPreferences, AdaptationResult, TripItinerary, DailyItinerary, Activity, 
    ChecklistItem
)
...
def generate_fallback_checklist(
    ...
    items = [
        ChecklistItem(task="Verify passport validity (minimum 6 months remaining)", category="Documents"),
        ChecklistItem(task=f"Check local entry requirements and visa regulations for {dest}", category="Documents"),
        ChecklistItem(task="Pack adapters compatible with local power outlets", category="Packing"),
        ChecklistItem(task="Acquire small bills in local currency for markets and tipping", category="Finance")
    ]

    # Adapt packing/etiquette list to diagnostic results
    if challenge == "Beginner":
        items.extend([
            ChecklistItem(task="Carry a printed basic greeting cheat sheet", category="Etiquette"),
            ChecklistItem(task="Confirm shoulders and knees are covered for temple visits", category="Packing"),
            ChecklistItem(task="Download local offline maps and translation apps", category="Packing")
        ])
    elif challenge == "Intermediate":
        if "Safety" in focus:
            items.extend([
                ChecklistItem(task="Review common dining faux-pas (e.g. chopstick positioning rules)", category="Etiquette"),
                ChecklistItem(task="Watch video on avoiding common tourist scams/overcharging zones", category="Etiquette"),
                ChecklistItem(task="Bring slip-on shoes for frequent shoe removal requirements", category="Packing")
            ])
        else:
            items.extend([
                ChecklistItem(task="Learn basic polite dining phrases ('Thank you for the food')", category="Etiquette"),
                ChecklistItem(task="Bring slip-on shoes for temple and home entrances", category="Packing")
            ])
    else: # Advanced
        items.extend([
            ChecklistItem(task="Memorize advanced local social gestures and greetings", category="Etiquette"),
            ChecklistItem(task="Bring a small traditional gift (Omiyage/present) from home if meeting local hosts", category="Packing"),
            ChecklistItem(task="Read brief guide on regional dialects or local folklore", category="Etiquette")
        ])
