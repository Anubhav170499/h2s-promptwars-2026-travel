...
from travelpilot.models import (
    DiagnosticAnswer, DiagnosticQuestion, AdaptationResult, 
    TravelPreferences, BudgetFeasibility, Activity, SubstitutionResponse
)

# Standard template diagnostic questions
...
def score_diagnostic(answers: List[DiagnosticAnswer]) -> AdaptationResult:
    # Build maps of questions
...
def check_budget_feasibility(preferences: TravelPreferences, estimated_cost: float) -> BudgetFeasibility:
...
def adapt_activity_substitution(
    ...
    elif "budget" in reason_lower or "cost" in reason_lower or "overrun" in reason_lower:
        new_act = Activity(
